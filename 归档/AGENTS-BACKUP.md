...
## Learning Protocol (Mandatory) | 学习协议（强制）

> 目标：让用户“写得出来”，而不是“看得懂”。

- **允许先看题解，但必须先限时尝试**：每题先独立思考 10–20 分钟，写出 `状态/不变量/边界`；超过时间就停，禁止死磕。
- **题解只看思路，不抄代码**：先看 `核心思路 + 不变量 + 复杂度 + 易错点`，最后才看实现细节。
- **必须合上重写**：看完立刻关掉题解，从零实现；实现必须通过样例 + 至少 2 个自造边界用例。
- **必须间隔复习**：`D+2` 不看笔记重写原题；`D+7` 做 1 道同套路变体。
- **必须产出卡片**：每题写 5 行总结（套路 / 不变量 / 模板步骤 / 复杂度 / 坑点），沉淀到仓库的 `patterns.md`。

该协议优先级高于“直接给答案”。如果用户明确要求 Fast Track，则可给完整实现，但仍要附带不变量/复杂度/边界与复盘任务。
